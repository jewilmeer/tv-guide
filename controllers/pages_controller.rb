class PagesController < ApplicationController
  ...
  # hompage
  def index
    @featured_programs = (Episode.next_airing.distinct_program_id('episodes.airs_at').limit(3).map(&:program) | Episode.by_created_at(:desc).distinct_program_id('episoides.created_at').limit(3).map(&:program))
